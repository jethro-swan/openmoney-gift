...
module.exports = Backbone.View.extend({
  ...
  render: function(){
    ...
    Self.$el.html(Self.template(Self.merchant.toJSON()));
    ...
    Self.$('#forgot').validate({
        ...
        messages: {
            merchantname: {
                required: "Merchant name or Email is required.",
  ...
  next: function( event ) {
    ...
    if( Self.$('#forgot').valid() ) {
      var user = {};
      if(Self.$('#merchantname').val().indexOf('@') !== -1){
        user.email = Self.$('#merchantname').val();
      } else {
        user.merchantname = Self.$('#merchantname').val();
      }
      ...
      options.url = '/V1/merchants/forgot';
